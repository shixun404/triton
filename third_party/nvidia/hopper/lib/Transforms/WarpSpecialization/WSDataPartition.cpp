...
static Operation *sliceOp(Operation *op, int offset, IRMapping &mappings,
  ...
  if ((dim == DataPartitionScheme::noOpPartitionDim) ||
    ...
    sliceOp(transOp.getSrc(), offset, mappings, reverseMappings,
            partitionScheme);
    Value newSrc = mappings.lookupOrNull(transOp.getSrc());
    assert(newSrc && "trans src not sliced");
    ...
    if (auto srcEnc = srcTy.getEncoding()) {
      ...
      if (failed(iface->inferTransOpEncoding(srcEnc, srcTy.getShape(), order,
                                             dstEnc, op->getLoc()))) {
    ...
    auto newOutTy =
        RankedTensorType::get(newOutShape, oldOutTy.getElementType(), dstEnc);
